...
class HHModel:
    def __init__(self, dt = 0.01, niter = 10000):
        self.gK_max = 0.36
        self.gNa_max = 1.20
        self.vK = -77
        self.vNa = 50
    ...
    def g_total(self):
        return self.gNa() + self.gK() + self.gL
    ...
    def _v(self, it):
        v_inf = self.v_inf(it)
        tauv = self.tauv(it)
        return v_inf + (self.v_hist[it]-v_inf)*np.exp(-self.dt/tauv)
    
    def alpha_m(self):
        return 0.1*(self.v + 40)/(1-np.exp(-(self.v + 55)/10))
    ...
    def __call__(self, I_inp):
        self.I_inp = I_inp
        self.I = self.I*I_inp
        for it in range(self.niter):
            self.gNa_hist[it] = self.gNa()
            self.gK_hist[it] = self.gK()
            self.gTot_hist[it] = self.g_total()
            self.v = self._v(it) 
